...
recursive('D:/Kanban/Projects_Gali/prodCatData_Master_May/catalogueData/device/tablets/', function (err, files) {
...
function readdeviceDetails(deviceJSON){
    ...
    var otherProductsArray = deviceJSON["fulfillmentData"]["otherProducts"],simSKU = "xx",simName=  "xx";
    for(var otherPCount=0;otherPCount<otherProductsArray.length;otherPCount++){
      if(otherProductsArray[otherPCount]["productType"] == "POS"){
            simSKU = otherProductsArray[otherPCount]["productId"];
            simName = otherProductsArray[otherPCount]["productName"];
          break;
      }
    }
    var deviceObj = {
...
function generateExcelFile(collection){
    ...
    ws.Cell(1,4).String('SKU');
    ws.Cell(1,5).String('StockInfo');
    ws.Cell(1,6).String('Sim Type');
    ws.Cell(1,7).String('Sim Name');
    for(var skuCountLength = 0;skuCountLength < collection.length;skuCountLength++){
        ...
        ws.Cell(row,4).String(collection[skuCountLength]["sku"]);
        ws.Cell(row,5).String(collection[skuCountLength]["StockInfo"]);
        ws.Cell(row,6).String(collection[skuCountLength]["simSKU"]);
        ws.Cell(row,7).String(collection[skuCountLength]["simName"]);
    ...
    wb.write("ExcelOutput/Tablets_SIM_Details_v1.xlsx",function(err){ 
        console.log("done");
    });
