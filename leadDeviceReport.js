...
function readdeviceDetails(deviceJSON){
   ...
   var images = deviceJSON["images"];
   var listHalfImages = 1;
   if(images["standard"]["listHalf"] )
   listHalfImages = images["standard"]["listHalf"];
    var deviceObj = {
                   "guid":deviceJSON["id"],
                   "sku": deviceJSON["sku"]["code"],
                   "model": deviceJSON["model"],
                  "images": listHalfImages,
   ...
   if(deviceJSON["id"] == deviceJSON["leadModelInFamily"])
...
function generateExcelFile(collection){
    ...
    ws.Cell(1,4).String('images');
    ws.Cell(1,5).String('Lifecycle');
    ws.Cell(1,6).String('ConsumerNew');
    ws.Cell(1,7).String('ConsumerUpgrade');
    ws.Cell(1,8).String('VoiceNew');
    ws.Cell(1,9).String('VoiceUpgrade');
    ws.Cell(1,10).String('Pay&Go');
    ws.Cell(1,11).String('PayMonthly');
    ws.Cell(1,12).String('StockInfo');
    ws.Cell(1,13).String('OnStockPots');
    ws.Cell(1,14).String('disableClickAndCollect');
    ws.Cell(1,15).String('disableClickAndCollectNow');
    ws.Cell(1,16).String('Category');
    for(var skuCountLength = 0;skuCountLength < collection.length;skuCountLength++){
        ...
        ws.Cell(row,4).String(collection[skuCountLength]["images"].toString());
        ws.Cell(row,5).String(collection[skuCountLength]["lifecycle"]);
        ws.Cell(row,6).String(collection[skuCountLength]["ConsumerNew"]);
        ws.Cell(row,7).String(collection[skuCountLength]["ConsumerUpgrade"]);
        ws.Cell(row,8).String(collection[skuCountLength]["VoiceNew"]);
        ws.Cell(row,9).String(collection[skuCountLength]["VoiceUpgrade"]);
        ws.Cell(row,10).String(collection[skuCountLength]["payGo"]);
        ws.Cell(row,11).String(collection[skuCountLength]["PayMonthly"]);
        ws.Cell(row,12).String(collection[skuCountLength]["StockInfo"]);
        ws.Cell(row,13).String(collection[skuCountLength]["OnStockPots"]);
        ws.Cell(row,14).String(collection[skuCountLength]["disableClickAndCollect"]);
        ws.Cell(row,15).String(collection[skuCountLength]["disableClickAndCollectNow"]);
        ws.Cell(row,16).String(collection[skuCountLength]["category"]);
    ...
    wb.write("ExcelOutput/deviceDetails_Report_15_10_2015.xlsx",function(err){
    console.log(err);
        console.log("done");
    });
