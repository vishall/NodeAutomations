...
function addDeviceFilestoAccy(accyJSON,count,file,newPathsContainer){
    ...
    for(var devicePathsObjectCount= 0;devicePathsObjectCount<devicePathsObject.length;devicePathsObjectCount++){
        var accyRecommededPhonesContainer = accyJSON["recommendedForPhones"];
        var deviceJSONFilePath = "${linkTo('"+devicePathsObject[devicePathsObjectCount]["url"]+"')}";
        for(var accyRecommededPhonesContainerCount =0;accyRecommededPhonesContainerCount<accyRecommededPhonesContainer.length;accyRecommededPhonesContainerCount++){
             if(accyJSON["recommendedForPhones"][accyRecommededPhonesContainerCount] === deviceJSONFilePath)
              accyJSON["recommendedForPhones"].remove(accyRecommededPhonesContainerCount);
        }
