...
var O2_500Mins_Bolton = {
    "type": "bolton",
    "id": "${idOf('/boltons/ngtrs-unlimited.json')}"
};

var O2_limited_Bolton = {
    "type": "bolton",
    "id": "${idOf('/boltons/ngtrs-limited.json')}"
};
...
function loadLoyaltyDiscountsJSON(){   
    ...
    recursive('D:/productCatalogueData_Masternew/catalogueData/plan/q116/november/', function (err, files) {
        var jsonFileCount = 0;
        var index = 0;
        ...
        jsonFiles.forEach(function(file) {
            ...
            var newPathsContainer = [] , newPathsContainer1=[];
            var json;
            if(newSearch != null){
                ...
                for(var jCount =0;jCount<uniqueArray.length;jCount++){
                   ...
                   newPathsContainer.push(uniqueArray[jCount]);
				    newPathsContainer1.push(uniqueArray[jCount]);
...
//console.log(newPathsContainer);
            newPathsContainer.push("${idOf('/boltons/ngtrs-unlimited.json')}");
			newPathsContainer1.push("${idOf('/boltons/ngtrs-limited.json')}");
            var tariffPID = json["productID"];
            var flag = true;
            for(var tariffPIDCount = 0; tariffPIDCount < pidContainerLength ; tariffPIDCount++){
					if(json["subType"]=="Voice"&&json["callTime"]["value"]=="Unlimited"){
					     if(o2BoltonPIDContainer[tariffPIDCount]["pid"] == tariffPID){
						  add500O2Bolton(json,file,newPathsContainer);
						  flag = false;
						  break;
						}}else if(json["subType"]=="Voice"&&json["callTime"]["value"]!="Unlimited"){
							 addlimitedO2Bolton(json,file,newPathsContainer1);
						  flag = false;
						  break;
...
function loadPIDSforO2Bolton(){
    ...
    var workbook = XLSX.readFile('ExcelOutput/O2toO2_500_Mins_compat.xlsx');
    var sheet_Min_Count = 2,sheet_Max_Count = 25;
...
function addlimitedO2Bolton(tariffJSON,file,newPathsContainer){
    if(tariffJSON["relationships"]){
       tariffJSON["relationships"].push(O2_limited_Bolton);
    }
    else{
      tariffJSON["relationships"] = relationships; 
    }
